/**
 * Shared Pexels search + scoring (browser + Vercel api/pexels.js).
 * Kept under /lib so the serverless bundle always includes it.
 */
export async function resolvePexelsImageUrlForQuery(query, apiKey, fetchImpl = fetch) {
  const q = query.replace(/\s+/g, " ").trim().slice(0, 100);
  if (!q || !apiKey) return null;

  const words = q.split(/\s+/).filter(Boolean);
  const short = words.slice(0, 3).join(" ") || "gift";

  try {
    return (
      (await searchPexelsOnce(q, "landscape", apiKey, fetchImpl)) ||
      (await searchPexelsOnce(q, null, apiKey, fetchImpl)) ||
      (await searchPexelsOnce(short, "landscape", apiKey, fetchImpl)) ||
      (await searchPexelsOnce(short, null, apiKey, fetchImpl))
    );
  } catch {
    return null;
  }
...
async function searchPexelsOnce(query, orientation, apiKey, fetchImpl) {
  const q = query.replace(/\s+/g, " ").trim().slice(0, 100);
  if (!q) return null;
